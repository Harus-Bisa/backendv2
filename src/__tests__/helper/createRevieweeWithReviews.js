...
async function createRevieweeWithReviews(reviewCount = 0, authorId = undefined) {
	if (!authorId) {
		const {
			userEmail,
			userPassword,
  ...
  for (i = 0; i < reviewCount; i++) {
    ...
    await revieweeService.createReview(authorId, newReviewee.revieweeId, review);
  }

  const {reviewee} = await revieweeService.getRevieweeById(newReviewee.revieweeId, true, authorId);
